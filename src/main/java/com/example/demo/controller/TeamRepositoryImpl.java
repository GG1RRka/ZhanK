...
public class TeamRepositoryImpl {
    final static String ALL_TEAM_NATIVE_QUERY =
            "select id, name, country, totalEarnings, currency from teams";
    ...
    public void insert(Team team) {
        em.createNamedQuery(Team.INSERT, Team.class).setParameter("id", team.getId())
                .setParameter("name", team.getName())
                .setParameter("country", team.getCountry())
                .setParameter("totalEarnings", team.getEarnings())
                .setParameter("currency", team.getCurrency()).executeUpdate();
    }
    ...
    @Transactional(readOnly = true)
    public List<Team> findByAttrEarnings(Long totalEarnings) {
        List<Team> teams = em.createNamedQuery(Team.FIND_BY_EARNINGS, Team.class).setParameter("totalEarnings", totalEarnings).getResultList();
        return teams;
    }

    @Transactional(readOnly = true)
    public List<Team> findByAttrCurrency(String currency) {
        List<Team> teams = em.createNamedQuery(Team.FIND_BY_CURRENCY, Team.class).setParameter("currency", currency).getResultList();
        return teams;
    }
